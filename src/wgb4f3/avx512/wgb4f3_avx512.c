...
// Currently implement the logic PARALLEL_INNER mode, aka. parallelize inside L2
// tile blocks
...
static int64_t get_tiles_l2_blk(const int64_t N, const int64_t C,
  ...
  int64_t tiles_l2_blk = TILE_L2_BLK_MAX_LARGE;
...
WinogradOptParams init_winconv_4x3_params(const int N, const int C, const int K,
  ...
  param.tiles_l2_blk = get_tiles_l2_blk(N, C, K, H, W);
      ...
      for (int64_t ic_blk = ic_l2; ic_blk < ic_l2 + l2_ic_compute_padded;
        ...
        for (int64_t tile_blk = tile_l2; tile_blk < tile_l2 + l2_tiles_compute;
          ...
          const int64_t blk_tile_compute =
              MIN(tile_l2 + l2_tiles_compute - (tile_blk - t), TILE_KERNEL_BLK);
      ...
      for (int64_t oc_l2 = 0; oc_l2 < param.padded_oc;
        ...
        for (int64_t ti = 0; ti < TILE_IN_H * TILE_IN_W; ++ti) {
          for (int64_t oc_blk = oc_l2; oc_blk < oc_l2 + l2_oc_compute;
            ...
            int64_t kernel_oc_len = ROUND_UP(blk_oc_compute, KERNEL_ONE_REG);
            float *src_trans_in =
            ...
            float *gemm_out;
            if (param.override_gemm) {
              gemm_out = gemm_out_buf + ti * l2_tiles_compute * l2_oc_compute +
        ...
        if (is_last_ic) {
          for (int64_t oc_blk = oc_l2; oc_blk < oc_l2 + l2_oc_compute;
            ...
            for (int64_t tile_blk = tile_l2;
              ...
              if (param.override_gemm) {
                gemm_out = gemm_out_buf + l2_tiles_compute * (oc_blk - oc_l2) +
              ...
              if (oh_len == TILE_OUT_H && ow_len == TILE_OUT_W) {
                winograd_b4f3_dsttrans_fp32_avx512(gemm_out, gemm_out_ti_stride,
                                                   dst_w * KERNEL_ONE_REG,
                                                   dst_trans_buf, blk_dst);
              } else {
                float *dst_buf = dst_trans_buf + param.blk_matmul_out_len;
                winograd_b4f3_dsttrans_fp32_avx512(gemm_out, gemm_out_ti_stride,
                                                   TILE_OUT_W * KERNEL_ONE_REG,
                                                   dst_trans_buf, dst_buf);
                winograd_b4f3_store_dst_fp32_avx512(
                    dst_buf, oh_len, ow_len, dst_w * KERNEL_ONE_REG, blk_dst);
              }
            }
          }
        }
